...
var stateSums = dataset.bankBalances.reduce(function (accumulator, current){
  ...
  if (accumulator[current.state]){
    ...
    accumulator[current.state] = Math.round(accumulator[current.state] * 100) / 100;
  }else{
...
var sumOfHighInterests = Object.keys(stateSums).filter(function (account){
  // filter out the states not included // 
  console.log(account);
  return account !== "WI" && account !== "IL" && account !== "WY" && account !== "OH" && account !== "GA" && account !== "DE";
}).reduce(function (accumulator, current){
